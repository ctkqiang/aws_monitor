...
function getDateRange(granularity: PeriodGranularity): {
  ...
  switch (granularity) {
    ...
    case 'WEEKLY':
      ...
      start = new Date(today.getTime() - 83 * 86400000);
  ...
  if (prevStart < maxLookback) {
    prevStart = maxLookback;
  }
...
async function fetchCost(
...
): Promise<{ resultsByTime: ResultByTime[]; groups: Group[] }> {
  ...
  return {
    ...
    groups: (res.GroupDefinitions ? res.ResultsByTime?.[0]?.Groups || [] : []),
  };
}
...
function computePeriodTotal(resultsByTime: ResultByTime[]): number {
  let total = 0;
  for (const period of resultsByTime) {
    total += parseFloat(period.Total?.UnblendedCost?.Amount || '0');
  }
  ...
  return useQuery<BillingOverview>({
    ...
    queryFn: async () => {
      try {
        ...
        Logger.info(TAG, 'Fetching billing data', {
          granularity,
          startDate,
          endDate,
          ceGranularity,
        ...
        Logger.info(TAG, 'Billing data fetched', {
          currentTotal,
          previousTotal,
          trend,
          trendPercent,
        ...
        return {
          ...
          dailyBreakdown,
          services,
          monthlyEstimate: granularity === 'MONTHLY'
            ? Math.round(currentTotal * 100) / 100
            : Math.round((currentTotal / Math.max(dailyBreakdown.length, 1)) * 30 * 100) / 100,
        };
      } catch (e: any) {
        Logger.logError(TAG, 'GetCostAndUsage failed', e);

        if (e?.name === 'AccessDeniedException' || e?.message?.includes('not authorized')) {
          Logger.warn(TAG, 'Cost Explorer access denied — requires IAM permission ce:GetCostAndUsage');
          return {
